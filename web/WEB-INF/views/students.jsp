            ...
            <div style=" margin-top: 10px;">
                <%
                    if (showSearchResults) {
                        int records = sr.countNode();
                %>
                <%= records%> record(s) 
                <% } else if (mergedList != null && !mergedList.isEmpty()) {
                    int records = mergedList.countNode();
                %>
                <%= records%> record(s)
                <% } else { %>
            ...
            <table border="1" style="width:80%; margin-top: 10px" class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="/UniversityManagement/studentServlet?action=displayStudentCourses&id=<%= s.getId()%>&status=<%= s.getStatus()%>&programId=<%= s.getProgrammeId()%>">Assign Courses</a></td>
